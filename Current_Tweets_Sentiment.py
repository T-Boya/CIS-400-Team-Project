...
#Using twitter stream timeline to find the current tweet with filtered terms in n number or n sec. 
...
def Current_Tweets_Sentiment(listOfTerms, numberOfTweet):
    ...
    for tweet in stream:
        try:
            ...
            #Making sure to only count one user IDs.
            if tweet['id'] not in list_Of_IDs:
                ...
                listlist_Of_IDs.append(tweet['id'])
    ...
    classifier, wf = create_classifier() #take too long to run
...
if __name__ == "__main__":
    ...
    dem, rep = Current_Tweets_Sentiment(Biden, 100)
